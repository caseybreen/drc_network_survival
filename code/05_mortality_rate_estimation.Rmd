...
```{r}
main_estimate_neighbor <- calculate_exposure_and_death(death_df = death_df, survey_df = survey_df, type = "neighbor", bootstrap = 1000)
main_estimate_kin <- calculate_exposure_and_death(death_df = death_df, survey_df = survey_df, type = "kin", bootstrap = 1000)

overall_estimate <- bind_rows(main_estimate_neighbor, main_estimate_kin) %>% 
  mutate(method = type) %>% 
  group_by(method) %>% 
...
```{r}
main_estimate_neighbor_zs <- calculate_exposure_and_death(death_df = death_df, survey_df = survey_df, type = "neighbor", subpopulation = "zone_de_sante_name", bootstrap = 1000)
main_estimate_kin_zs <- calculate_exposure_and_death(death_df = death_df, survey_df = survey_df, type = "kin", subpopulation = "zone_de_sante_name", bootstrap = 1000)

## zone de sante 
zone_de_sante <- bind_rows(main_estimate_neighbor_zs, main_estimate_kin_zs) %>% 
  mutate(method = type) %>% 
...
```{r}
## main estimates 
main_estimate_neighbor_monthly <- calculate_exposure_and_death(death_df = death_df, survey_df = survey_df, type = "neighbor", bootstrap = 1000, monthly = T)
main_estimate_kin_monthly <- calculate_exposure_and_death(death_df = death_df, survey_df = survey_df, type = "kin", bootstrap = 1000, monthly = T)

## estimates monthly 
results_monthly_df_zs <- bind_rows(main_estimate_neighbor_monthly, main_estimate_kin_monthly) %>% 
...
main_plot_monthly <-  results_monthly_df_zs %>% 
  filter(month != as_date("2023-06-01")) %>% 
...
```{r}
## main estimates 
main_estimate_neighbor_monthly_zs <- calculate_exposure_and_death(death_df = death_df, survey_df = survey_df, type = "neighbor", bootstrap = 1000, subpopulation = "zone_de_sante_name", monthly = T)

main_estimate_kin_monthly_zs <- calculate_exposure_and_death(death_df = death_df, survey_df = survey_df, type = "kin", bootstrap = 1000, subpopulation = "zone_de_sante_name", monthly = T)

## estimates monthly 
results_monthly_df_zs_monthly <- bind_rows(main_estimate_neighbor_monthly_zs, main_estimate_kin_monthly_zs) %>% 
  filter(!is.na(death_rate)) %>% 
  mutate(method = type) %>% 
