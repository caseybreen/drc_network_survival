...
cdr_monthly_plot <- cdr_estimates %>% 
  filter(health_zone == "pooled" & month != "pooled" & survey == "quota" & type == "blended" & weights == "weight_ipw") %>% 
...
```{r}
## visualize results 
cdr_pooled_heath_zones_plot <- cdr_estimates %>% 
  filter(health_zone != "pooled" & month == "pooled" & survey == "quota") %>% 
...
## visualize results 
cdr_network_comparisons <- cdr_estimates %>% 
  filter(health_zone == "pooled" & month == "pooled") %>% 
  filter(survey == "probability" | survey == "quota" & weights == "weight_ipw") %>% 
