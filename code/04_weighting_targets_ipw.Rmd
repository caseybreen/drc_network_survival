...
household_sample_respondent_df <- household_sample_respondent %>%
  select(uuid_col, cooking_fuel, material_house, modern_fuel_type, manufactured_material_house, radio, bed, `_id`, health_zone)

household_sample_roster_df_individual <- household_sample_roster %>%
  ...
  select("_submission__uuid", gender, age_class)
...
hh_weighting_micro_df <- household_sample_roster_df_individual %>% 
  left_join(household_sample_roster_df, by = c('_submission__uuid')) %>% 
  left_join(household_sample_respondent_df, by = c('_submission__uuid' = 'uuid_col'))
