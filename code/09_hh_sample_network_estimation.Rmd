...
cdr_pooled_hz_point <- networksurvival::compute_cdr_comprehensive(death_df = hh_sample_deaths, survey_df = hh_sample_respondent, weight_col = "weight_poststrat", subpopulation = "health_zone", bootstrap = NA)

cdr_pooled_hz_boostrap <- networksurvival::compute_cdr_comprehensive(death_df = hh_sample_deaths, survey_df = hh_sample_respondent, weight_col = "weight_poststrat", subpopulation = "health_zone", bootstrap = 100)
...
```{r}
## estimate point estimates 
cdr_pooled_hz_point_nonprob <- networksurvival::compute_cdr_comprehensive(death_df = quota_sample_deaths, survey_df = quota_sample_survey, weight_col = "weight_poststrat", subpopulation = "health_zone", bootstrap = NA)

cdr_pooled_hz_boostrap_nonprob <- networksurvival::compute_cdr_comprehensive(death_df = quota_sample_deaths, survey_df = quota_sample_survey, weight_col = "weight_poststrat", subpopulation = "health_zone", bootstrap = 100)
...
final_results_plot <- cdr_pooled_hz_point_uncertainty_nonprob %>%
  ...
  filter(health_zone %in% c("Kalemie", "Nyemba")) %>%
  filter(weights == "poststrat") %>%
