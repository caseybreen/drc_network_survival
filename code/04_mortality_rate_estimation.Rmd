---
title: "Estiamte mortality rates"
---

Summarize: Estimate mortality rates 
...
ggsave(plot = main_estimate_plot, filename = here("figures/main_estimate_plot.png"), width = 4, height = 4)
```


```{r}
main_estimate_zs <- networksurvival::estimate_death_rate(death_df = death_df, survey_df = survey_df, weight_col = "weight_poststrat", subpopulation = "zone_de_sante_name", bootstrap = 1000)

## zone de sante 
zone_de_sante <- main_estimate_zs %>% 
  mutate(method = type) %>% 
...
# Calculate the average death rate for kin and neighbor for each bootstrap iteration
average_death_rates <- main_estimate_zs %>%
  group_by(zone_de_sante_name, bootstrap_iter) %>% 
  ...
  summarize(lower = quantile(death_rate, 0.025),
            upper = quantile(death_rate, 0.975),
            mean = mean(death_rate)) %>% 
  mutate(method = "blended")


zone_de_sante_plot <- zone_de_sante %>% 
    bind_rows(average_death_rates) %>% 
...
## zone de sante 
main_estimate_monthly_df <- main_estimate_monthly %>% 
  mutate(method = type) %>% 
...
# Calculate the average death rate for kin and neighbor for each bootstrap iteration
main_estimate_monthly_blended_df <- main_estimate_monthly %>%
    group_by(month, bootstrap_iter) %>% 
  summarize(death_rate = mean(death_rate, na.rm = TRUE)) %>%
  ungroup() %>% 
  group_by(month) %>% 
  summarize(lower = quantile(death_rate, 0.025),
...
main_estimate_monthly_df %>% 
  bind_rows(main_estimate_monthly_blended_df) %>% 
...
main_plot_monthly <-  main_estimate_monthly_df %>% 
  bind_rows(main_estimate_monthly_blended_df) %>% 
  filter(month != as_date("2023-06-01")) %>% 
