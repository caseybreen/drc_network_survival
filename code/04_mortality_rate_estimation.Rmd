...
cdr_pooled_bootstrap <- networksurvival::compute_cdr_comprehensive(death_df = death_df,
                                                                        survey_df = survey_df,
                                                                        weight_col = "weight_poststrat", bootstrap = 100) 
...
ggsave(plot = cdr_pooled_heath_zones_plot, filename = here("figures/cdr_pooled_heath_zones_plot.png"), width = 6, height = 4)
```
...
## results monthly df 
main_plot_monthly_zs <- cdr_monthly_hz_point_uncertainty %>% 
...
preregistered_estimates <- bind_rows(cdr_pooled_hz_point_uncertainty, cdr_pooled_point_uncertainty) %>% 
  filter(type != "household") %>% 
  mutate(geography = case_when(
    is.na(zone_de_sante_name) ~ "Pooled",
    TRUE ~ zone_de_sante_name
  )) %>% 
  select(geography, network_tie = type, weights, death_rate, death_rate_lower, death_rate_upper) %>% 
  arrange(desc(geography))

# Convert to LaTeX table
preregistered_estimates_tex <- xtable(preregistered_estimates, caption = "", label = "table:preregister")

# Print the LaTeX code with customized format
print.xtable(preregistered_estimates_tex, include.rownames = FALSE,
             ...
             booktabs = TRUE,
             file = here("tables/preregistered_estimates.tex")) # Use booktabs for better formatting
```

```{r}
## pre-registered estimates 
preregistered_estimates_monthly <- cdr_monthly_point_uncertainty %>% 
  filter(type != "household") %>% 
  mutate(geography = "Pooled") %>% 
  select(geography, month, network_tie = type, weights, death_rate, death_rate_lower, death_rate_upper) %>% 
  arrange(desc(geography)) %>% 
  mutate(month = as.character(month))


# Convert to LaTeX table
preregistered_estimates_monthly_tex <- xtable(preregistered_estimates_monthly, caption = "", label = "table:preregister_monthly")

# Print the LaTeX code with customized format
print.xtable(preregistered_estimates_monthly_tex, include.rownames = FALSE,
             hline.after = c(-1, 0, nrow(data)), # Add horizontal lines at specific positions
             tabular.environment = "tabular",
             floating.environment = "table",
             floating = TRUE,
             latex.environments = "center",
             booktabs = TRUE,
             file = here("tables/preregistered_estimates_monthly.tex")) # Use booktabs for better formatting
...
```{r}
## pre-registered estimates 
preregistered_estimates_monthly_hz <- cdr_monthly_hz_point_uncertainty %>% 
  filter(type != "household") %>% 
  select(health_zone = zone_de_sante_name, month, network_tie = type, weights, death_rate, death_rate_lower, death_rate_upper) %>% 
  mutate(month = as.character(month)) %>% 
  arrange(desc(health_zone))

# Convert to LaTeX table
preregistered_estimates_monthly_hz_tex <- xtable(preregistered_estimates_monthly_hz, caption = "", label = "table:preregister_monthly")

# Print the LaTeX code with customized format
print.xtable(preregistered_estimates_monthly_hz_tex, include.rownames = FALSE,
             hline.after = c(-1, 0, nrow(data)), # Add horizontal lines at specific positions
             tabular.environment = "tabular",
             floating.environment = "table",
             floating = TRUE,
             latex.environments = "center",
             booktabs = TRUE,
             file = here("tables/preregistered_estimates_monthly_hz.tex")) # Use booktabs for better formatting
```
